...
//sales
func (controller *WebControllers) AddSales(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	var userLogin models.LoginUser
	session := sessions.Start(w, r)
	userLogin.UserID = session.GetString("userid")
	userLogin.LevelUser = session.GetString("leveluser")
...
func (controller *WebControllers) DataSales(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	var userLogin models.LoginUser
	session := sessions.Start(w, r)
	userLogin.UserID = session.GetString("userid")
	userLogin.LevelUser = session.GetString("leveluser")
...
//company
func (controller *WebControllers) AddCompany(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	var userLogin models.LoginUser
	session := sessions.Start(w, r)
	userLogin.UserID = session.GetString("userid")
	userLogin.LevelUser = session.GetString("leveluser")
...
func (controller *WebControllers) DataCompany(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	var userLogin models.LoginUser
	session := sessions.Start(w, r)
	userLogin.UserID = session.GetString("userid")
	userLogin.LevelUser = session.GetString("leveluser")
	...
	if r.Method == "POST" {
		...
		err4 = htmlTemplate.ExecuteTemplate(w, "base", "")
		if err4 != nil {
			http.Error(w, err4.Error(), http.StatusInternalServerError)
			f.Println(err4.Error())
		}
...
//pic
func (controller *WebControllers) AddPIC(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	var userLogin models.LoginUser
	session := sessions.Start(w, r)
	userLogin.UserID = session.GetString("userid")
	userLogin.LevelUser = session.GetString("leveluser")
...
func (controller *WebControllers) DataPIC(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	var userLogin models.LoginUser
	session := sessions.Start(w, r)
	userLogin.UserID = session.GetString("userid")
	userLogin.LevelUser = session.GetString("leveluser")
...
func (controller *WebControllers) AddCommentTask(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	...
	if r.Method == "POST" {
		...
		var labelDeadline = r.FormValue("labelCheckbox")
		var commentNotes = r.FormValue("commentNotes")
		//var checkBoxDateDeadline = r.FormValue("customCheckboxDeadline")

		if labelDeadline == "YES" {
			var dateDeadline = r.FormValue("dateDeadline")
			db.AddTaskComment(taskId, dateDeadline, userLogin.UserID, commentNotes, labelDeadline)
		} else {
			db.AddTaskComment(taskId, "", userLogin.UserID, commentNotes, labelDeadline)
		}
		...
		var tempCommentTask models.TempTask

		getTaskID_sql := f.Sprintf("SELECT TOP 1 task_comment_id FROM task ORDER BY task_comment_id DESC ")
		...
		if err6 != nil {
			...
			if rows6.Next() {
				...
				tempCommentTask.TaskID = utilities.GetNextCode(task_id, "TCM")
			} else {
				tempCommentTask.TaskID = "TCM0001"
			}
		...
		if err4 != nil {
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			f.Println("Ini Errornya dari parse file: " + err4.Error())
			return
		}
