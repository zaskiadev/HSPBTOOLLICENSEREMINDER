...
func AddTaskComment(taskID string, dateDeadLine string, userCommentTask string, commentTaskNotes string, changeDateDeadline string) bool {
	ConnString := f.Sprintf("server=%s;user id=%s;password=%s;port=%d;database=%s;sslmode=disable", Server, User, Password, Port, Db)
	...
	var success bool
	status := "created"
	if changeDateDeadline == "YES" {
		insertProduct_sql := f.Sprintf("INSERT INTO task_comment (task_comment_id, user_id_create_task, user_id_delegation_task, date_created_task,date_deadline_task,company_id_destination_task,sales_id_destination_task,pic_id_destination_task, task_notes,status_task) "+
			"VALUES ('%s' , '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s') ", taskID, userCreatedTask, userDestinationTask, currentTime.Format("2006-01-02 15:04:05"), dateDeadLine, companyTask, salesCompanyTask, picCompanyTask, taskNotes, "created")
		rows, err := conn.Query(insertProduct_sql)
		println(insertProduct_sql)
		if err != nil {
